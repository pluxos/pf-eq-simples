...
specs = do
          describe "Teste de funcões simples" $ do
            ...
            it "Soma tres números " $ property $
              \x y z -> somaTresNumeros x y z `shouldBe` x + y + z
            it "Calcula a area do quadrado " $
              areaQuadrado 2 `shouldBe` 4
            it "Calcula a area do quadrado " $ property $
              \x -> areaQuadrado x `shouldBe` x**x
            it "Calcula a area do retangulo " $
              areaRetangulo 2 3 `shouldBe` 6
            it "Calcula a area do retangulo " $ property $
              \x y -> abs $ areaRetangulo x y `shouldBe` abs $ x * y
            it "Calcula a area do círculo " $ property $
              \x -> areaCirculo x `shouldBe` abs $ pi * x**2
            it "Calcula a area do triâgulo " $ property $
              \x y -> abs $ areaTriangulo x y `shouldBe` abs $ x*y/2
            it "Calcula a area do trapézio " $ property $
              \x y z -> abs $ areaTrapezio x y z `shouldBe` abs $ (x + y)*z/2
            it "Calcula a hipotenusa " $ property $
              \x y -> abs $ hipotenusa x y `shouldBe` sqrt (x*x + y*y)
            it "Converts C to F " $ 
          ...
          describe "Teste de funcões com if-then-else" $ do
            it "Retorna o maior de 3 números" $ property $
              \x y z -> maiorDeTres x y z `shouldBe` maximum x:y:z:[]
